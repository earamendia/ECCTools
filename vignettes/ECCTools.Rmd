...
1. To formulate a Multi-Regional Physical Supply Use Table (MR-PSUT) framework representing the ECC. In this case, imports are treated as supply coming from an industry located in a different region. This option is the closest representation of physical flows.
2. To a formulate a PSUT framework following the Domestic Technology Assumption (DTA). The DTA assumes that imports are produced within the region that imports them. Although not a realistic assumption, it may prove to be a helpful simulation.
...
1. First, the IEA data may be loaded as usually done with the `IEATools` package (see ....).
2. Second, specific flows may be modified, for instance, to aggregate regions, to locate them in a different matrix (in particular, to the newly introduced *balance* matrix), or to perform any other modification.
...
Subsequent ECC analysis can be conducting using the `Recca` package (see ...).
...
1.  *Using the balancing matrix*: Shows how to relocate flows into the newly defined balancing matrix and what the downstream consequences are. See ...
2.  *Domestic Technology Assumption*: Shows how to formulate the Domestic Technology Assumptions and which conditions need to be filfilled to formulate such an assumption. See ...
3.  *Global Market Assumption*: Shows how to formulate the Global Market Assumption. See ...
4.  *Bilateral Trade Assumption*: Shows how to provide bilateral trade data to formulate the Bilateral Trade Assumption. See ...


## Demonstration example: the AB world

To illustrate uses of the package, we introduce a fictitious world consisting of two countries (A and B): welcome to the AB world! The Energy Conversion Chain of country A is shown in Figure 1.
...
Then, Figure 2 shows the Energy Conversion Chain of country B.
...
The fact that these ECCs, when taken together, are unbalanced, is an attempt to represent *fidèlement* the features of IEA data. Indeed, this type of unbalances appear when combining the whole IEA dataset, and our AB world example will show how such inconsistencies are dealt with when working with real IEA data.

Now, some guesses about why such inconsistencies may appear in IEA data:

* World coverage incomplete. Some small, and some not that small countries, are not covered in IEA data. When these countries trade energy products with other countries, then that trade has to translate into a sink or source of energy products (provided the partner countries are covered by the data set.
...
* Differences in energy products classification and naming between countries may also lead to discrepancies in the global figures of imported and exported figures. We note however that the IEA attempts to standardise the naming convention, but such errors remain likely on a minor scale. *a petite échelle.*
...
readr::read_rds("data/tidy_AB_data.rds") %>% 
  dplyr::glimpse()
```
